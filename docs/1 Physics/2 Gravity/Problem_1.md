...
# === Relative Mass Bar Chart (Logarithmic Scale) ===
...
plt.yscale('log')  # Logarithmic scale to make small values visible
plt.ylabel('Mass (in Earth Masses, log scale)')
plt.title('Sun vs Earth Mass (Relative Comparison)')

# Add value labels on top of bars
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2.0, yval * 1.2, f"{yval:.0f}", ha='center')
